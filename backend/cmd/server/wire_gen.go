...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	feishuNotificationOutboxRepository := repository.NewFeishuNotificationOutboxRepository(db)
	feishuEventReceiptRepository := repository.NewFeishuEventReceiptRepository(db)
	notificationPreferenceRepository := repository.NewNotificationPreferenceRepository(db)
	channelMonitorRepository := repository.NewChannelMonitorRepository(client, db)
	feishuAPIKeyRequestRepository := repository.NewFeishuAPIKeyRequestRepository(db)
	usageLogRepository := repository.NewUsageLogRepository(client, db)
	feishuChatBindingRepository := repository.NewFeishuChatBindingRepository(db)
	feishuNotificationService := service.ProvideFeishuNotificationService(settingRepository, feishuUserIdentityRepository, feishuNotificationOutboxRepository, feishuEventReceiptRepository, userRepository, userSubscriptionRepository, apiKeyRepository, notificationPreferenceRepository, channelMonitorRepository, feishuAPIKeyRequestRepository, apiKeyService, usageLogRepository, feishuChatBindingRepository, groupRepository)
	...
	contentModerationService := service.NewContentModerationService(settingRepository, contentModerationRepository, contentModerationHashCache, groupRepository, userRepository, proxyRepository, apiKeyAuthCacheInvalidator, emailService, v...)
	...
	accountResetRepository := repository.NewAccountResetRepository(db)
	accountResetService := service.ProvideAccountResetService(accountResetRepository, accountRepository, billingCacheService, subscriptionService, feishuNotificationService)
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, accountResetService)
	...
	openAIOAuthHandler := admin.ProvideOpenAIOAuthHandler(openAIOAuthService, adminService, openAIQuotaService, rateLimitService, accountResetService)
	...
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, aliyunCaptchaService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, feishuNotificationService, totpService, userService)
	...
	quotaStatusHandler := handler.NewQuotaStatusHandler(quotaStatusService, apiKeyService)
	clientSubscriptionLimitsService := service.NewClientSubscriptionLimitsService(accountRepository, openAIQuotaService)
	clientSubscriptionLimitsHandler := handler.NewClientSubscriptionLimitsHandler(clientSubscriptionLimitsService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, quotaStatusHandler, clientSubscriptionLimitsHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, apiKeyRuntimeService, opsService, settingService, compositeRouteResolver, redisClient)

...
// NormalizeAndValidateWebAuthnConfig applies the same relying-party security
// checks to deployment and database-backed runtime configuration.
func NormalizeAndValidateWebAuthnConfig(cfg *WebAuthnConfig) error {
	if cfg == nil || !cfg.Enabled {
		return nil
	}
	cfg.RPDisplayName = strings.TrimSpace(cfg.RPDisplayName)
	cfg.RPID = strings.ToLower(strings.TrimSpace(cfg.RPID))
	cfg.RPOrigins = normalizeStringSlice(cfg.RPOrigins)
	...
	if strings.Contains(cfg.RPID, "://") || strings.ContainsAny(cfg.RPID, "/:") {
	...
	for i, origin := range cfg.RPOrigins {
		...
		if host != cfg.RPID && !strings.HasSuffix(host, "."+cfg.RPID) {
			return fmt.Errorf("webauthn.rp_origins entry %q is not within relying party ID %q", origin, cfg.RPID)
		}
		cfg.RPOrigins[i] = u.Scheme + "://" + u.Host
	}
	return nil
}
...
func (c *Config) Validate() error {
	...
	if err := NormalizeAndValidateWebAuthnConfig(&c.WebAuthn); err != nil {
		return err
	}

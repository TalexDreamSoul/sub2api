...
func TestRecordCyberPolicyEvent_RespectsContentModerationScope(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// This fork applies moderation bans through ContentModerationUserBan
			// (repo.UpsertUserBan) and marks the log AutoBanned, rather than flipping
			// user.Status directly; assert the fork's ban mechanism.
			logs := repo.snapshotLogs()
			if tt.wantBanned {
				require.NotEmpty(t, logs)
				require.True(t, logs[len(logs)-1].AutoBanned, "in-scope cyber hit at ban threshold must auto-ban")
			} else {
				for _, l := range logs {
					require.False(t, l.AutoBanned)
				}
			}
		})
	}
